...
- https://dev.to/heroku/how-to-manage-your-secrets-with-git-crypt-56ih (Простая версия, достаточно её)
- https://buddy.works/guides/git-crypt (Опциональная продвинутая версия с GPG ключами)
...
Предоставляю пример моих таблиц для [Sqlitebrowser](https://sqlitebrowser.org/) в [файле preduzetnik.sqlite3](secret_repository/preduzetnik.sqlite3), открывать через Sqlitebrowser
Структура таблиц легко меняется через GUI без SQL кода! Вместе с миграцией данных!

#### Моя Entity Relationship Diagrams
...
- Любые платежи следует логике что мы сначала имеем инвойс документ клиенту, после чего происходит платеж. Оба документа вносяться в соответствующую строку таблицы.
...
- в таблицу `service_payment` я вношу платежи от имени компании, эти платежи идет с налоговым вычетом :)
...
- Доступен универсальный язык дял запроса прибыли итого или с групировкой по месяцам/годам, а так же расходов на налоги или сервисы. SQL язык богат и удобен.
